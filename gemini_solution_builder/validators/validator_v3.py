...
from gemini_solution_builder.validators.schemas import SchemaV3
from gemini_solution_builder.validators import ValidatorV2
...
class ValidatorV3(ValidatorV2):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.deployment_tasks_path = join_path(
            self.solution_path, 'deployment_tasks.yaml')
        self.network_roles_path = join_path(
            self.solution_path, 'network_roles.yaml')
        self.node_roles_path = join_path(
            self.solution_path, 'node_roles.yaml')
        self.volumes_path = join_path(
            self.solution_path, 'volumes.yaml')
    ...
    def check_schemas(self):
        logger.debug('Start schema checking "%s"', self.solution_path)
